...
const LABELS={
  cost:'Cost', alt:'海拔曲线', season:'行程拆分', weather:'天气',
  routes:'4 趟行程', switchPlan:'行程对比', places:'沿途地点',
  ourPins:'收藏点', bookings:'预订', prepList:'行前待办',
  reconcile:'对账', fx:'汇率', rules:'高原反应规则',
  compare:'4 趟季节冲突', aboutPage:'关于此页'
};

const TXT={
  season:
    '4 地无共同最佳月份，拆分 4 趟，各择当地季节。'
    +'<br><br>秘鲁旱季 5–9 月；乌尤尼镜面季 = 雨季 12–4 月；'
    +'巴塔哥尼亚徒步季 = 南半球夏季 11–3 月；哥伦比亚/巴西干季 12–3 月。'
    +'秘鲁旱季与乌尤尼镜面季相反，强行合并须牺牲一项。'
    +'<br><br>拆分优势：单趟节奏可放缓，约 2 周，落地缓冲 2 日，不赶行程。',

  alt:
    '横条 = 当晚睡眠海拔。<b>规则：低海拔睡眠+高海拔活动</b>：日间可达 4000 m 以上，'
    +'夜间海拔单日涨幅 ≤500 m。'
    +'<br><br>红色标记 = 当晚海拔涨幅过大，须关注身体反应。',

  wxNote:
    '降水单位 mm。7 天预报出发前两周开始查看。上栏为去年同期实测，'
    +'用于判断该时间窗雨量+气温。安第斯山区昼夜温差极大：日间 20°C，夜间接近 0°C 为常态。'
    +'数据 Open-Meteo（CC BY 4.0）。',

  itinHint: '点任意一天，地图跟随。左侧数字 = 当晚睡眠海拔，红色 = 涨幅超标。',

  pinsEmpty:
    '暂无收藏点。分享行程码给同行者，新增餐厅/住宿/机位实时同步。'
    +'地图长按可直接落点。',
  ...
  prepIntro:
    '按当前行程筛选。南美硬性门槛：马丘比丘门票分线路限额、'
    +'托雷斯德帕因营地/山屋须先预订、部分线路查验黄热病疫苗。',

  budgetNote:
    '单位加元，1 CAD ≈ 0.73 USD。数字为估算，可调整。'
    +'<br><br>南美机票占比高于亚洲：境内航段多且贵。'
    +'巴塔哥尼亚尤甚：El Calafate 至 Punta Arenas 仅限飞行或长途大巴。'
    +'反之当地吃住成本低于日本，秘鲁午市套餐（menú）约十几索尔。',
  ...
  aboutFoot:
    '共享数据对同码用户可见。坐标为示意精度，海拔为公开常见值，班次+门票以官方当日公告为准。'
};

/* ── 高原反应规则 ── */
const RULES=[
 ['低海拔睡眠+高海拔活动','日间 4000 m 以上无碍，关键在夜间海拔。超 3000 m 后，单夜涨幅须控制在 500 m 内。'],
 ['先住圣谷，后上库斯科','库斯科 3400 m，圣谷乌鲁班巴 2871 m/奥扬泰坦博 2792 m。落地库斯科机场直接下探圣谷过夜，优于市区硬扛，同时更近马丘比丘火车站。'],
 ['高强度项目排后段','彩虹山 5200 m/瓦卡奇纳/萨尔坎泰类，排在适应期 3 日后。高反与体能无关，运动员同样中招。'],
 ['前 48 小时','限酒/限大餐/多饮水。古柯茶（mate de coca）随处可得。Diamox 须提前咨询医生。'],
 ['下降为唯一有效对策','症状不缓解即下撤。阿瓜斯卡连特斯 2040 m/圣谷 2800 m 为现成退路。']
];

/* ── 四趟的季节冲突 ── */
const COMPARE=[
 ['秘鲁','旱季 5–9 月，晴天多，山路通畅。1–3 月雨季，印加古道 2 月整月封闭维护。'],
 ['玻利维亚 乌尤尼','镜面季须赶雨季 12–4 月，最佳 2–3 月。深入盐湖/红湖白湖需干季 6–8 月。两种玩法互斥。'],
 ['巴塔哥尼亚','11–3 月南半球夏季。12–2 月旺季，风力+人流最大；3 月风小人少，秋色初现，多数人认为最佳月份。'],
 ['巴西 / 哥伦比亚','哥伦比亚干季 12–3 月。巴西南部夏季 12–3 月，伊瓜苏水量雨季后 3–5 月最壮观。']
];
...
const REG={per:{n:'秘鲁',hex:'#C4392D'},bol:{n:'玻利维亚',hex:'#D99A2B'},
           ...
           cit:{n:'城市',hex:'#7A6A8C'}};
const P={};
const def=(id,zh,es,lat,lng,r,note,alt,kind)=>P[id]={id,zh,es,lat,lng,r,note,alt,kind};
...
def('larco','拉尔科博物馆','Museo Larco',-12.0722,-77.0700,'per','前哥伦布时期陶器收藏，馆藏质量很高。花园餐厅可吃午饭。',100);
...
def('aguas','阿瓜斯卡连特斯','Aguas Calientes',-13.1547,-72.5253,'per','2040 米。马丘比丘山脚的镇子，除了住宿和餐厅没有别的。住一晚可赶第一班车上山。',2040);
...
def('perito','佩里托莫雷诺冰川','Glaciar Perito Moreno',-50.4967,-73.1377,'pat','少数还在前进的冰川之一。栈道分几层，能听到崩塌的巨响。也可走冰上徒步（minitrekking）。',300);
def('chalten','El Chaltén','El Chaltén',-49.3315,-72.8863,'pat','400 米，阿根廷的徒步之都。所有步道从镇上直接起步，不用开车不用报团，这是它最好的地方。',400);
def('fitzroy','菲茨罗伊 Laguna de los Tres','Laguna de los Tres',-49.2717,-73.0083,'pat','全程 20 公里往返，最后一公里陡升 400 米。晴天是巴塔哥尼亚最有名的画面。风大就别上最后那段。',1170);
...
def('greyglacier','格雷冰川','Glaciar Grey',-51.0333,-73.2000,'pat','W 线西端，可坐船靠近冰川正面。',100);
...
/* — 枢纽 — */
def('scl','圣地亚哥机场','SCL Santiago',-33.3930,-70.7858,'cit','进出智利和巴塔哥尼亚的主要中转点。','way');
def('eze','布宜诺斯艾利斯','Buenos Aires EZE',-34.8222,-58.5358,'cit','进出阿根廷的枢纽。转机时间够进城吃顿饭。','way');
def('lim','利马机场','LIM Jorge Chávez',-12.0219,-77.1143,'cit','从温哥华出发多数在洛杉矶、休斯敦或墨西哥城转一次。','way');
...
A:{k:'A',gr:'一',name:'秘鲁 · 圣谷与马丘比丘',met:'14 DAYS · 旱季 6–8月',hex:'#C4392D',tag:'第一趟',
  ...
  pitch:'14 天，节奏宽松。核心路线自低到高：利马海平面 → 圣谷 2871 m → 马丘比丘 2430 m → 库斯科 3399 m。\n\n'
    +'常规做法：落地直接入住库斯科，随后头痛 2 日。此线反向操作：圣谷先住 2 晚，'
    +'待适应后再上城区，彩虹山（5000 m 级）排最后。',
  cons:'旱季即旺季，马丘比丘门票+火车须提前久订。\n\n'
    +'6–8 月安第斯昼夜温差极大：日间 20°C 出头，夜间接近 0°C。圣谷多数房源无暖气。',
  ...
  days:[
   ...
   {d:'06.13',w:'日',t:'利马 · 海平面缓一天',r:'per',stay:'利马',alt:0,note:'时差和高原不要一起扛。今天留在海平面。',stops:[
     {t:'上午',p:'larco',n:'拉尔科博物馆',s:'前哥伦布时期陶器，花园餐厅可吃午饭'},
   ...
   {d:'06.21',w:'一',t:'彩虹山 或 乌曼台湖',r:'per',stay:'库斯科',alt:3399,note:'适应到第九天才做这个。凌晨三四点出发，天气不好就是一片灰，别有执念。',stops:[
     {t:'04:00',p:'rainbow',n:'彩虹山 5200 米',s:'可骑马上最后一段'},
   ...
   {d:'06.23',w:'三',t:'飞回利马',r:'per',stay:'利马',alt:0,note:'回到海平面，睡一晚会好。',stops:[{t:'—',p:'lima',n:'米拉弗洛雷斯',s:'最后吃一顿正经的'}]},
...
B:{k:'B',gr:'二',name:'玻利维亚 · 乌尤尼镜面',met:'12 DAYS · 镜面季 2–3月',hex:'#D99A2B',tag:'第二趟',
  ...
  pitch:'12 天，为镜面而来。薄水层须降雨后形成，窗口 12–4 月，2–3 月概率最高。\n\n'
    +'全程 3600 m 以上，4 趟中高反压力最大。'
    +'前段安排拉巴斯+的的喀喀湖缓步适应，不直冲乌尤尼。',
  cons:'镜面效果靠运气。雨量过大盐湖封路，无雨则无水膜。无法提前锁定。\n\n'
    +'3 日团第 2 晚住红湖附近（4200 m 以上），条件简陋，为全程最艰苦一晚。'
    +'规避方案：仅报 1 日团，当日往返乌尤尼镇。',
  ...
  days:[
   ...
   {d:'02.21',w:'一',t:'拉巴斯 · 坐缆车',r:'bol',stay:'拉巴斯',alt:3640,note:'缆车为当地公交，也是最省力的观光方式，免爬坡。',stops:[
   ...
   {d:'02.24',w:'四',t:'回拉巴斯 → 飞乌尤尼',r:'bol',stay:'乌尤尼镇',alt:3670,note:'也可坐夜班大巴，省一晚住宿但很累。',stops:[
   ...
   {d:'02.26',w:'六',t:'盐湖 · 日出团 或 三日团出发',r:'bol',stay:'乌尤尼镇 / 山里',alt:3670,note:'想深入红湖白湖就今天出发三日团，那两晚条件简陋且更高。只想看镜面留在镇上。',stops:[
     {t:'日出',p:'uyuni',n:'日出镜面',s:''},
     {t:'—',p:'incahuasi',n:'鱼岛（水浅时可到）',s:'仙人掌石岛'}]},
   {d:'02.27',w:'日',t:'红湖 与 火烈鸟（三日团）',r:'bol',stay:'红湖附近',alt:4278,note:'今晚睡 4200 米以上，是全程最高的一晚。做一日团这天留在乌尤尼镇休息。',stops:[
...
C:{k:'C',gr:'三',name:'巴塔哥尼亚 · 冰川与徒步',met:'16 DAYS · 南半球夏 11–3月',hex:'#2E8FA6',tag:'第三趟',
  ...
  pitch:'16 天，阿根廷+智利各半程。全程低海拔，无高反顾虑，'
    +'风力是主要变量：巴塔哥尼亚强风可致寸步难行。\n\n'
    +'El Chaltén 段体验最佳：步道均从镇上直接起步，免开车免报团，'
    +'当日视天气选路线。',
  cons:'托雷斯德帕因营地+山屋须提前预订方可入园，旺季提前数月订满，禁止野营。\n\n'
    +'境内航段贵且班次少。El Calafate–Punta Arenas 仅限飞行或长途大巴。\n\n'
    +'11 月风力最大。风小人少可选 3 月，代价：日照时间缩短。',
  ...
  days:[
   ...
   {d:'11.16',w:'四',t:'托雷湖',r:'pat',stay:'El Chaltén',alt:400,note:'18 公里往返，平缓，当作热身。',stops:[{t:'—',p:'torre',n:'Laguna Torre',s:''}]},
   ...
   {d:'11.23',w:'四',t:'W 线 · 格雷冰川',r:'pat',stay:'园区山屋',alt:100,note:'可加钱坐船靠近冰川正面。',stops:[{t:'—',p:'greyglacier',n:'Glaciar Grey',s:''}]},
...
D:{k:'D',gr:'四',name:'哥伦比亚 · 或 巴西',met:'12 DAYS · 干季 12–3月',hex:'#3F8F5B',tag:'第四趟',
  ...
  pitch:'12 天，全程低海拔，4 趟中强度最低。\n\n'
    +'哥伦比亚线：波哥大 → 麦德林 → 咖啡三角 → 卡塔赫纳，气候/城市/海岸各异，距离紧凑。\n'
    +'巴西线：里约 → 帕拉蒂 → 伊瓜苏，重心自然+海滩。2 选 1，不叠加。',
  cons:'哥伦比亚部分区域治安须留意：白天移动，夜间使用打车软件，不拦街车。\n\n'
    +'巴西线伊瓜苏水量 3–5 月最大，1 月偏小；反之 3–5 月里约天气逊于 1 月。',
...
const TODOS=[
 ...
 {id:'s5',n:'看医生问高原药',s:'Diamox（乙酰唑胺）需处方，要提前问过。高反和体能无关',due:'出发前 2 月',v:'AB'},
...
const STAY={
 ...
 '拉巴斯':{area:'La Paz, Sopocachi / Zona Sur',
  opts:[{n:'Zona Sur（海拔更低）',tier:'$$',pick:true,q:'Zona Sur La Paz hotel',
    why:'比市中心低三百米，第一晚睡这边明显好受。缆车能上市区。'},
  ...
  food:[{n:'Salteñas（只在早上）',jp:'salteñas La Paz',why:'汤汁很多的酥皮包，中午前就卖完。'},
   {n:'Mercado Lanza 的摊子',jp:'Mercado Lanza La Paz',why:'便宜且本地。肠胃敏感第一天先别试。'}]},
 ...
 'El Chaltén':{area:'El Chaltén, Argentina',
  ...
  food:[{n:'La Cervecería 一类的精酿馆',jp:'cerveceria El Chalten',why:'走完一天的常规动作。'},
   {n:'超市自备干粮',jp:'supermercado El Chalten',why:'步道上没有任何补给点，水可在溪流补。'}]},

 'Puerto Natales':{area:'Puerto Natales, Chile',
  opts:[{n:'镇中心',tier:'$$',pick:true,q:'Puerto Natales hotel',
    why:'租装备、买补给、确认园区预订都在这里。很多旅舍可寄存大件行李。'}],
 ...
 '园区山屋':{area:'Torres del Paine, refugios',
  ...
  food:[{n:'山屋的套餐',jp:'',why:'贵但省事，可只订早晚餐。'},
 ...
 '麦德林':{area:'Medellín, El Poblado / Laureles',
  opts:[{n:'Laureles',tier:'$$',pick:true,q:'Laureles Medellin hotel',
   ...
   {n:'El Poblado',tier:'$$',q:'El Poblado Medellin hotel',
    why:'夜生活集中，游客也最多。'}],
  food:[{n:'Bandeja paisa',jp:'bandeja paisa Medellin',why:'分量极大，两个人可分一份。'}]},
