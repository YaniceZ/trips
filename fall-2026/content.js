...
const LABELS={
  cost:'Cost', peak:'变色窗口', drive:'驾驶强度', season:'最佳时机', weather:'天气',
  routes:'4 条线路', switchPlan:'线路对比', places:'沿途地点',
  ourPins:'收藏点', bookings:'预订', prepList:'行前待办',
  reconcile:'对账', fx:'汇率', rules:'赏枫 3 条规则', compare:'4 条线路对比',
  aboutPage:'关于此页'
};

const TXT={
  peak:
    '横条 = 各线路<b>变色高峰期</b>，竖线 = 当前所选出发日+结束日。'
    +'重叠越多，满树秋色概率越高。<br><br>'
    +'改右上角日期，此图自动重算。',

  season:
    '加拿大秋色自北向南推进，跨度 5–6 周。'
    +'<br><br>育空+北部最早：9 月中下旬结束。安大略 Algonquin+魁北克 Charlevoix：'
    +'9 月底–10 月初。Cape Breton 高地：10 月头两周。新斯科舍海岸线最晚：10 月中下旬。'
    +'<br><br>实际时间因天气浮动 5–10 天。冷夜+晴天出红色，暖而多云的 9 月偏黄。'
    +'出发前 1–2 周查各省实时色彩地图，优于历史均值。',

  drive:
    '横条 = 每日驾驶小时数。>5 小时标红。'
    +'<br><br>赏枫本质是公路旅行，景观在路上而非终点，每日车程不宜过满。'
    +'预留随时靠边停车的余量。',

  wxNote:
    '降水单位 mm。7 天预报出发前一周开始每日查看。上栏为去年同期实测。'
    +'秋季东部早晚接近 0°C，日间可达 15°C 以上，分层穿衣。雨后落叶速度加快。'
    +'数据 Open-Meteo（CC BY 4.0）。',

  itinHint: '点任意一天，地图跟随。左侧数字 = 当日驾驶小时数，红色 >5 小时。',

  pinsEmpty:
    '暂无收藏点。分享行程码给同行者，新增观景点/餐厅/民宿实时同步。'
    +'地图长按可直接落点。',
  ...
  prepIntro:
    '按当前线路筛选。赏枫季住宿最紧张：热门区域提前 6–8 周预订。',

  budgetNote:
    '单位加元。数字为估算，可调整。'
    +'<br><br>4 条线路均从温哥华飞出，机票为最大支出。租车在赏枫季涨价，'
    +'感恩节长周末（10 月）尤甚。'
    +'<br><br>住宿差异最大：Algonquin/Charlevoix 乡村旅舍旺季翻倍，'
    +'育空为淡季价格（夏季旅游团已离场）。',

  noteHint: '记实地信息：观景台停车位/最佳光线时段/闭店/路线修正。显示在描述上方。',

  aboutPage:
    '行程码 = 数据视图。同码用户共享收藏点。新码 = 空白行程，供他人独立使用。',
  aboutFoot:
    '共享数据对同码用户可见。坐标为示意精度，变色窗口为常年区间，实际以各省当季色彩地图为准。'
};
...
/* ── 关于赏枫的三件事 ── */
const RULES=[
 ['颜色自北向南推进','育空 9 月中结束，Algonquin/Charlevoix 9 月底–10 月初，Cape Breton 高地 10 月头两周。同期不同地，实为不同阶段。'],
 ['年际浮动 5–10 天','冷夜+晴天出最艳红色；暖而多云的 9 月偏黄。强风暴雨可致高峰期骤止。出发前 1–2 周查实时色彩地图，不依赖常年区间。'],
 ['10 月感恩节长周末为高峰','加拿大感恩节为 10 月第 2 个周一。该周末全国出游，热门区域住宿价格+路上车流翻倍。能避开则避开。']
];

const COMPARE=[
 ['甲 · Algonquin + Muskoka','经典森林+湖泊组合。Highway 60 走廊路边可观，免徒步。深入可划独木舟进内湖。近多伦多，机票+租车成本最低。'],
 ['乙 · 魁北克 + Charlevoix','秋色外含法语古城+沿河小镇。Route 362 段为全国最佳赏枫公路之一。内容最丰富，非纯自然行程。'],
 ['丙 · Cape Breton + 新斯科舍','Cabot Trail = 海岸线+高地组合，色彩映射大西洋。10 月上旬 Celtic Colours 音乐节同期举办。高峰期晚于另两条 1–2 周。'],
 ['丁 · 育空','荒野尺度显著不同，白桦+杨树金色铺满山谷，兼具极光机会。9 月下旬已近尾声，去晚见落叶后褐色。']
];
...
A:{k:'A',gr:'甲',name:'Algonquin + Muskoka',met:'6 DAYS · 安大略',hex:'#C8412B',tag:'最经典',
  ...
  pitch:'经典森林+湖泊组合。Highway 60 走廊 56 km，路边观景点密集分布，'
    +'免徒步可观。深入画面 → 划独木舟进内湖，20 分钟脱离人群。\n\n'
    +'距多伦多 2.5 小时，4 条线中机票+租车成本最低，节奏最松。',
  cons:'Algonquin 为 4 条线中游客最集中路线。周末 Highway 60 停车场 10 点满，工作日出行。\n\n'
    +'公园内信号弱。住宿提前订：赏枫季 Huntsville 一带房源紧张。',
  ...
  days:[
   ...
   {d:'09.28',w:'',t:'Algonquin · Highway 60 走廊',r:'on',stay:'Huntsville',drive:3,
    ...
    note:'公园需买日票，西门进。今天以路边观景为主。',stops:[
...
B:{k:'B',gr:'乙',name:'魁北克 + Charlevoix',met:'8 DAYS · 魁北克',hex:'#E0A526',tag:'最丰富',
  ...
  pitch:'秋色外含法语古城+完整沿河小镇线。'
    +'Route 362（Baie-Saint-Paul 至 La Malbaie 段）为全国公认最佳赏枫公路之一。\n\n'
    +'内容最丰富路线：老城/峡谷/缆车/果园/峡湾，每日画面各异。',
  cons:'驾驶时间超方案甲，8 天中 3 天超 3 小时。\n\n'
    +'日期若与 10 月第 2 个周一感恩节长周末重叠，Charlevoix 住宿翻倍且难订。',
  ...
  days:[
   ...
   {d:'09.30',w:'',t:'进 Charlevoix · Route 362',r:'qc',stay:'Baie-Saint-Paul',drive:2.5,
    sig:'河谷段时断时续。',note:'走老路 362，不走 138 高速。整条线的核心即此段。',stops:[
   ...
   {d:'10.01',w:'',t:'Hautes-Gorges 峡谷',r:'qc',stay:'La Malbaie',drive:3,
    sig:'公园内无信号。',note:'加拿大东部最深的峡谷之一，可坐船进去看两侧峭壁。',stops:[
   ...
   {d:'10.02',w:'',t:'Tadoussac 或 返程',r:'qc',stay:'魁北克城',drive:4,
    sig:'峡湾一带弱。',note:'看鲸季节到九月底基本结束，去晚了可能只剩风景。不去直接慢慢开回魁北克城。',stops:[
...
C:{k:'C',gr:'丙',name:'Cape Breton + 新斯科舍',met:'8 DAYS · 大西洋',hex:'#B85C2E',tag:'海岸线',
  ...
  pitch:'Cabot Trail = 海岸线+高地组合，色彩直映大西洋，'
    +'为另 3 条线所无的画面。10 月上旬 Celtic Colours 音乐节同期，'
    +'9 天内全岛数十村庄有演出。\n\n'
    +'南部 Lunenburg/Mahone Bay/Peggys Cove 顺路收录，秋色外含灯塔+渔村。',
  cons:'<b>高峰期晚于其余两条东部线 1–2 周</b>，Cape Breton 高地峰值通常在 10 月头两周。'
    +'日期落在 9 月下旬则偏早。\n\n'
    +'温哥华飞哈利法克斯通常转机 1 次，全程 8–10 小时，为 4 条线中飞行时间最长。',
  ...
  days:[
   {d:'10.01',w:'',t:'温哥华 → 哈利法克斯',r:'city',stay:'哈利法克斯',drive:1,
    sig:'正常。',note:'通常要转一次，全程 8–10 小时。时差 4 小时。',stops:[
   ...
   {d:'10.04',w:'',t:'Cabot Trail 西段',r:'ns',stay:'Chéticamp',drive:3,
    sig:'高地公园内大段无信号。',note:'逆时针开，内侧靠山；想靠海就顺时针。',stops:[
     {t:'—',p:'cabottrail',n:'Cabot Trail',s:'三分之一穿过高地国家公园，需买日票'},
...
D:{k:'D',gr:'丁',name:'育空 · 荒野与极光',met:'6 DAYS · 育空',hex:'#6FA07A',tag:'看时机',
  ...
  pitch:'荒野尺度显著不同于其余 3 条：白桦+杨树金色铺满山谷，'
    +'背景为 Kluane 雪山+冰原。9 月下旬起夜长足够，兼具极光机会。\n\n'
    +'温哥华直飞白马市仅 2.5 小时，4 条线中飞行时间最短。',
  cons:'<b>时机窗口最窄</b>：育空秋色 9 月中结束，9 月下旬已近尾声，'
    +'去晚见落叶后褐色。赏枫须将日期提前至 9 月上中旬。\n\n'
    +'主要目标为极光则相反：越晚越佳，此时不应按赏枫排程。\n\n'
    +'Tombstone+Dawson City 位于极北，6 天无法容纳，须另加 2–3 天。',
  ...
  days:[
   {d:'09.12',w:'',t:'温哥华 → 白马市',r:'city',stay:'白马市',drive:0.5,
    sig:'市区正常。',note:'直飞 2.5 小时。今天轻松，晚上晴就先看一次极光。',stops:[
...
const TODOS=[
 ...
 {id:'f5',n:'买公园日票',s:'Algonquin、Cape Breton 高地、Hautes-Gorges 都要单独买票，部分可线上先买',due:'出发前 1 周',v:'ABC'},
 {id:'f6',n:'订独木舟 / 缆车 / 观光飞行',s:'Canoe Lake 租船、Tremblant 和 Le Massif 缆车、Kluane 冰原飞行都提前',due:'出发前 1 周',v:'ABD'},
 ...
 {id:'f11',n:'注意驼鹿和野生动物',s:'清晨傍晚是活动高峰，公路上撞到驼鹿危险。天黑后减速',due:'出发前',v:'ABD'},
...
const STAY={
 ...
 '魁北克城':{area:'Vieux-Québec',
  ...
  food:[{n:'Petit-Champlain 一带',jp:'Petit Champlain Quebec restaurants',why:'游客多但确实好看。'},
   {n:'Poutine',jp:'poutine Quebec City',why:'来都来。'}]},
